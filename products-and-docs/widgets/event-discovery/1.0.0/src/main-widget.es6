class TicketmasterEventDiscoveryWidget {
  ...
  getDateFromPeriod(period){
    ...
    firstDay = today.toISOString().slice(0,19) + 'Z';
    if(period == "year" ) {
      lastDay = new Date(today.valueOf()+24*365*60*60*1000).toISOString().slice(0,19) + 'Z';
    }
    else if(period == "month") {
      lastDay = new Date(today.valueOf()+24*31*60*60*1000).toISOString().slice(0,19) + 'Z';
    }
    else if(period == "week") {
      lastDay = new Date(today.valueOf()+24*7*60*60*1000).toISOString().slice(0,19) + 'Z';
    } else {
      lastDay = new Date(today.valueOf()+24*60*60*1000).toISOString().slice(0,19) + 'Z';
    }
