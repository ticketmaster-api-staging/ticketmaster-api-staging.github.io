 ...
 * */
(function($) {
  let base = {}, // base object with parsed API data
  ...
  $(function() {
    let item = window.atob(getCookie('tk-api-email'));
  ...
  let getResponsiveId = function(width) {
    if (width < 768) {
      return 1;
    } else {
      if (width < 1200) {
  ...
  // initializes slider
  let sliderInitializator = function(slidesToShow) {
  ...
  let parseUrl = function() {
    ...
    if (location) {
      let querys = location.replace(/^\?/g, '').split('&');
      let obj = {};

      querys.map(function(e) {
        let a = decodeURI(e).split('=');
        obj[a[0]] = a[1];
      });
      return obj;
    }
   ...
   * @param obj
   */
  let setActiveDropdown = function(obj) {
    let apiSections = $('.api-dropdown .dropdown-toggle');
  ...
  let setListeners = function() {
    ...
    $('#clear-req-resp').on('click', function(e) {
      e.preventDefault();
      let container = $('#req-res-container'),
    ...
    $(window).on('resize', function() { // since slick-inbuilt responsive object has bugs when slides are added/removed - we have to reinit slider when screen is resized
      let newScreenWidth = Math.max(document.documentElement.clientWidth, window.innerWidth || 0);
      if (getResponsiveId(screenWidth) != getResponsiveId(newScreenWidth)) {
        screenWidth = newScreenWidth;
        slider.slick('unslick');
        initSlider();
      }
    }).on('login', function(e, data) {
      apiKey=data.key;
    });
  ...
  let setDropdownListeners = function() {
    $('body').on('click', '.nav', function(e) {
      let targetElement = $(e.target);
      let target = targetElement.parent();
  ...
  let primaryColumnSlideUp = function() {
  ...
  let primaryColumnSlideDown = function() {
  ...
  let dataJSONSlideDown = function() {
  ...
  let renderPrimaryColumn = function(method, callback) {
    let isPrimaryVisible = false,
    ...
    setTimeout(function() {
      ...
      for (let param in method.parameters) { // render new paramater fields
        let par = method.parameters[param],
          name = par.name,
  ...
  let addApiDropdown = function(apiName, selected) {
    let dropDown = $('<li class="dropdown api-dropdown"></li>'),
    ...
    for (let method in base[apiName]) {
      let li = $('<li role="presentation"><a class="select-default-method" api-name="'+ apiName + '" ' +
  ...
  let parseXMLDoc = function(xml) {
    ...
    APIs.each(function() {
      ...
      methods.each(function() {
        ...
        if (!base[category]) {
          base[category] = {};
        } // create new API in base object if there is none
  ...
  let setPaginationlistener = function() {
    let pageInput = $('#page');
  ...
  // column constructor
  let Column = function(configObject, responseObject, index, guId) {
    let self = this;
    ...
    self.init = function() {
      ...
      for (let i = 0; i < configObject.length; i++) { // iterate through method main subcolumns
        subcolumn = configObject[i]; // subcolumn
        listGroup = $('<div class="list-group"></div>'); // subcolumn future element
        let isPage = subcolumn.title.toLowerCase() === 'page';
        title = $([
        ...
        let destinationObject = subcolumn['path'] ? Object.byString(self.responseObject, subcolumn['path']) : self.responseObject; // object inside the response to iterate through
        ...
        if (subcolumn['expandsTo']) {
          let nextIndex = getNextColorIndex();
          let string = getFirstLowercase(subcolumn.title);
        ...
        if (subcolumn['map']) { // does subcolumn have latitude and longitude for map popup?
          let coordinatesDestination = subcolumn['map']['coordinates']['path'] ? Object.byString(destinationObject, subcolumn['map']['coordinates']['path']) : destinationObject, // if field has its additional path
          ...
          if (dataLat != 'undefined' && dataLng != 'undefined' && dataLat && dataLng) {
            subColumnMapImage = getMapImage(dataLat, dataLng);
          } // map image to be appended to column
          else { // if there are no coordinates available form map by address
            let address = '';
            for (let field in subcolumn['map']['address']) {
              let fieldDestination = subcolumn['map']['address'][field]['path'] ? Object.byString(destinationObject, subcolumn['map']['address'][field]['path']) : destinationObject; // if field has its own destination
        ...
        if (subcolumn['collection']) {
          let itemCount = 0, // variable to store items count
          ...
          for (let item in destinationDeep) { // iterate through response items collection
            let dimension = (subcolumn['fields'][0]['showDimension'] && isThumbnail ) ? ( ' ('+ destinationDeep[item]['width'] +'x'+ destinationDeep[item]['height'] +')' ) : '';
            let firstFieldId = subcolumn.fields[0].id;
            let selector = $('<a class="list-group-item"></a>');
            let content = '';
            ...
            if (isThumbnail) {
              let thumbnail = $('<img class="subcolumn-thumbnail" src="">');
          ...
          for (let field in subcolumn['fields']) {
            let destinationDeep = subcolumn['fields'][field]['path'] ? Object.byString(destinationObject, subcolumn['fields'][field]['path']) : destinationObject, // if field has its additional path
        ...
        if (subColumnMapImage && subcolumn['map']) { // append map image if there is any
          let imgListGroup = $('<div class="list-group"></div>'), // subcolumn future element
    ...
    self.setEventListeners = function() {
      self.column.on('click', function(e) {
        ...
        if ($(e.target).hasClass('expandable')) { // if it is a field and it expands to new column
          changeSelectedMethod(self.method, self.usedParams, function() { // change method selected above and fill in all parameter inputs
            ...
            setTimeout(function() { // timeout wait while slider animates
              ...
              let response = ($(e.target).attr('subcolumn-path') && index) ? (Object.byString(responseObject, $(e.target).attr('subcolumn-path') + '.' + index)): responseObject;
        ...
        if ($(e.target).hasClass('expand-new-method')) { // handles click event on More button
          e.preventDefault();
          let method = findMethodInBase($(e.target).attr('method')),
        ...
        if ($(e.target).hasClass('log-btn')) { // if log button is pressed
          e.preventDefault();
          spinner.show();
          let logItem = $('#req-res-container').find('[href='+ '#' + self.guId + ']');
  ...
  let getMapImage = function(lat, lng, address) {
    let coordinates = address ? address : (lat && lng ? (lat + ',' + lng) : '');
    if (coordinates) {
      let dividor = slider.slick('slickGetOption', 'slidesToShow'),
      ...
      let img = $('<img data-lat="' + lat + '" data-long="' + lng + '" data-address="' + address + '" class="api-column-map-image" src="' + url + '">');

      img
        .on('error', function() {
          let imgElem = $('.api-column-map-image');
  ...
  let showMapPopup = function(lat, lng, address) {
    let map,
      marker,
      mapEl = $('#map-popup'),
  ...
  let geocodeAddress = function(geocoder, map, address, callback) {
  ...
  let mapPopUpListener = function(e) {
    e.preventDefault();
    let lat = $(e.target).attr('data-lat') != 'undefined' ? parseFloat($(e.target).attr('data-lat')) : null,
  ...
  let changeSelectedMethod = function(method, paramObjectArray, callback) {
    let fillInInputVals = function() { // fill in all inputs
      for (let i = 0; i < paramObjectArray.length; i++) {
        let input = primaryColumn.find('#' + paramObjectArray[i]['id']);
        input.val(paramObjectArray[i]['value']);
        input.trigger('change');
      }
    };
    let scroll = function() { // scroll up to slider top and run callback
    ...
    if (selectedMethod.id != method.id) { // if new method is called rerender the parameters container
      renderPrimaryColumn(method, function() {
        let selectedGroup = $('.selected-group'),
  ...
  let sendPrimaryRequest = function(visible) {
    currentColumnColorIndex = getNextColorIndex();
    let url = formPrimaryURL(selectedMethod);
    if (!visible) {
      spinner.show();
    }
  ...
  let scrollToSlider = function(ms) {
    $('html, body').animate({ // scroll to slider
  ...
  let findMethodInBase = function(id) {
    for (let api in base) {
      for (let method in base[api]) {
        if (method === id) {
  ...
  let formPrimaryURL = function(method) {
    let params = getAllParameteres(), // parameter values from 1st column
  ...
  let getAllParameteres = function() {
    let params = primaryColumn.find('.parameter-item input'),
      paramArray = [];
  ...
  let highlightCode = function(selector, code) {
    let animTime = 100;
  ...
  let sendRequest = function(url, method, callback) {
    // spinner.show();
    ...
    $.ajax({
      ...
      success: function(response, textStatus, jqXHR) {
        let guid = guId(), // generate unique id for each accordion item
          selector,
          reqResItem,
          fadeInTime = 1000;
  ...
  let prettyfyJSON = function() {
    let textarea = document.getElementById('post-json-area'),
      ugly = textarea.value,
      obj = {},
      pretty = '';
  ...
  let formDeepLinkingUrl = function() {
    let location = window.location;
    let params = getAllParameteres();
    let querys = ['api=' + encodeURI(selectedMethod.category), 'method='+ encodeURI(selectedMethod.id)];
    for (let i in params) {
