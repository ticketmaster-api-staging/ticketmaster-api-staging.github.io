...
(function($) {
  jQuery.fn.applicationSelect = function(options) {
    ...
    return this.each(function() {
      ...
      let useData = apiKeyService.getApiKeysCookie('tk-api-apps');
      ...
      if ( $custom_select.find('select').length < 1 && useData && useData.length > 0) {
        let selectedKey = $(this).siblings('ul').find('li.custom_select__item-active').attr('data-value');
...
$(document).on('ready', function() {
  let listApiKeys = apiKeyService.getApiKeysCookie('tk-api-apps');
