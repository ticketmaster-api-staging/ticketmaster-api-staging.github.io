...
(function($) {
  $.fn.classificationSelector = function(options) {
    ...
    function updateTree(json) {
      let newJson = (json['_embedded'] && json['_embedded']['classifications']) ? setChildren(json['_embedded']['classifications']) : null;
    ...
    function rename(json) {
      ...
      let arr = JSON.stringify(json);// convert array to string
      arr = JSON.parse(arr);
      ...
      function replaceAtoB(arr, opt) {
        ...
        for (let i = 0; i<arr.length; i++) {
          ...
          if ( 0<genresArr.length ) {
            for (let ii = 0; ii < genresArr.length; ii++) {
              ...
              if ( 0<subgenresArr.length ) {
                for (let j = 0; j < subgenresArr.length; j++) {
    ...
    let renderResults = function(data, ulElement, errorMsg) {
      ...
      // show fail msg
      if (data === 'FAIL') {
        let msgErr = (errorMsg && errorMsg.responseJSON && errorMsg.responseJSON !== null )?errorMsg.responseJSON.errors[0].status : 'unknown',
    ...
    function setIdListener(e) {
      let selectedID = e.target.getAttribute('data-classificationId');
