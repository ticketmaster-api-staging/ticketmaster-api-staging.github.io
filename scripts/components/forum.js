...
function getThreads(cursor='') {
  let apiKey ='ohRvg9N3dieJJRshzJsaQbKJAO2ZNuAebul7MeXvICNIjNKjhuhPyTfSVzrP8y8M';
  $.ajax({
...
function drawThreads(threads, start, limit) {
  let content = '';
  let countPages = Math.ceil(parseFloat(threads.length / limit));
...
function findThreads(arr) {
  arThreadsFind = [];
  let limit = 8;
  let toFind = $('#search-inp').val().toUpperCase();
  for (let i in arr) {
