...
(function($) {
  let config = ['events', 'venues', 'attractions'],
    tagsIds ={}, initialVal={};
  ...
  $.fn.lazySelector = function(options) {
    ...
    let keyword = $form.find('#keyword'),
      ...
      apikey = function() {
        return apiKeyService.checkApiKeyCookie('tk-api-key') || apiKeyService.getApiExploreKey() || $('#w-tm-api-key').val() || defaultApiKey;
      },
    ...
    function formatDate(date) {
      ...
      let E = new Date(date.day).getDay();
    ...
    let mapPopUpListener = function(e) {
      e.preventDefault();
      let lat = $(e.target).attr('data-latitude') != 'undefined' ? parseFloat($(e.target).attr('data-latitude')) : null,
    ...
    function addMarker(mapCenter) {
      let marker = new google.maps.Marker({ // Create a marker and set its position.
        map: map,
        position: mapCenter,
     ...
     * @return {boolean} - done/fail
     */
    function submitForm(/* optional*/pageNumero) {
      pageNumero = parseInt(pageNumero);
      let url = ( isNaN(pageNumero) ) ? eventUrl + '?apikey=' + apikey() + '&keyword=' + keyword.val() :
    ...
    function getImageForEvent(images) {
      ...
      let src = (images[0].url ) ? (images[1] && images[1].url )? images[1].url : images[0].url : '';
      return src;
    }
    ...
    function insertImageTo(element, item ) {
      ...
      let img = $('<div style="background-image:url(' + src + ');" ></div>')
    ...
    function renderListEvents(items) {
      ...
      items.map(function(item) {
        ...
        let time = formatDate(currentEvent.date);
        let eventTime = $('<h4 class="event-time gray"/>')
          .text(time)
          .appendTo($wrapCol);
        /* add time end*/
        ...
        if (item.id) {
          ...
          let addToEl = {li: li, buttonSetIdWrapper: buttonSetId.parent()};
    ...
    function renderListVenues(items) {
      items.map(function(item) {
        ...
        let $wrapCol = $('<div class="event-text-wrapper clear-margin-left"/>')
          .appendTo(li);
        ...
        if (item) {
          let venue = item; // item._embedded.venues[0];
          let contryStateName = $('<h4/>')
        ...
        if (item.id) {
          ...
          if (venue.location && venue.location.latitude && venue.location.longitude) {
            let buttonMap = $('<button data-latitude=' + venue.location.latitude + ' data-longitude=' + venue.location.longitude + '/>')
      ...
      items.map(function(item) {
        ...
        if (item.id) {
          ...
          if (venue.location && venue.location.latitude && venue.location.longitude) {
            ...
            let buttonMap = $('<button style=\'float: right;\' data-latitude=' + venue.location.latitude + ' data-longitude=' + venue.location.longitude + '/>')
              .addClass('js_open-map_btn btn btn-transparent')
              .text('Show on map')
              .insertAfter(buttonSetId)
    ...
    let renderResults = function(data, ulElement) {
      let items;
    ...
    function addMsButton(el, data) {
      ...
      let buttonAddIdToList = $('<button data-id=' + data + '/>')
    ...
    function setIdsListener(e) {
      ...
      let selectedID = tagsIds[selector];
