...
function initMapLatLong() {
  ...
  let map_latlong = new google.maps.Map(document.getElementById('map_latlong'), {
    zoom: 2,
  ...
  let marker_latlong = new google.maps.Marker({
    icon: imageMarker,
  });
  ...
  let autocomplete = new google.maps.places.Autocomplete(pac_input);
  ...
  $('#js_widget_modal_map').on('shown.bs.modal', function() {
    ...
    let tmp_currentLatLng = document.getElementById('w-latlong').value.split(',');
    let currentLatLng = new google.maps.LatLng(parseInt(tmp_currentLatLng[0].replace(/\s+/g, '')), parseInt(tmp_currentLatLng[1].replace(/\s+/g, '')));
