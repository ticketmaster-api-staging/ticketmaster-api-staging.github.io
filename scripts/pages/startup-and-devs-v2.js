...
;(function(factory) {
  ...
  let SELECTOR_ACTIVE_GRID = '.active .grid';
  ...
  function initSlider($root, config) {
    if ($root.hasClass('slick-initialized') || $root.children().length < 2) {
      return false;
    }
  ...
  function setEventListeners($rootTarget) {
    ...
    $rootTarget
      .on('click', '#tab-dropdown .tab', function(e) {
        ...
        let $tab = $('.tab:contains("' + text + '")').tab('show');
  ...
  function reInitTabPane() {
    let CONFIG = PAGE_CONFIG.GALLERY;
  ...
  function bildTheGrid($grid) {
    let GRID_CONFIG = PAGE_CONFIG.GALLERY.GRID;
