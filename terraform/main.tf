...
resource "aws_elb" "app" {
  ...
  subnets                     = ["${split(",", module.networks.subnets[format("%s.%s.%s", var.aws_region, var.vpc, var.subnets_postfix)] )}"]
  ...
  listener {
    instance_port      = "${var.app_elb_port}"
    instance_protocol  = "${var.app_elb_protocol}"
